...
class DatabaseManager:
    def __init__(self, db_name: str = "with_list.db"):
    ...
    async def create_item(self, item_name: str, owner: str, description: Optional[str] = None) -> int:
        """Creation of a new record"""
        async with aiosqlite.connect(self.db_name) as db:
            time = datetime.now()

            cursor = await db.execute(
                'INSERT INTO wish_list (owner, item_name, description, created_at) VALUES (?, ?, ?, ?)',
                (owner, item_name, description, time)
            )
            await db.commit()
    ...
    async def get_list(self, owner: str) -> Iterable:
        """Read all records"""
        async with aiosqlite.connect(self.db_name) as db:
